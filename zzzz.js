...
app.post('/api/users/:_id/exercises', async (req,res)=> {
  ...
  const foundUser = await Person.findById(userId);
  ...
  //foundUser.push(savedExercise);
  await foundUser.save();
  res.json({ ...foundUser.toObject(), _id: savedExercise._id });
  /*res.send({
    username:foundUser.username,
    description,
    duration,
...
const userId = req.params._id;
  ...
  let exercise =await EX.find({filter}).limit(limit);
...
/*app.post('/api/users/:_id/exercises', async (req, res) => {
  ...
  try {
    const newExercise = new EX({
      user: userId, // Use the actual user reference (recommended)
...
// Save the exercise under the correct user
newExercise.user = user; // Assign the actual user reference
await newExercise.save();
res.json(newExercise.user)
   /*  if (!user) {
      return res.status(404).json({ error: 'User not found' });
    }
